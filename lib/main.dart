...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void getAllPost() {
    ...
    var sign=generateMd5({
      "currentPage": 0,
      "pageSize": 100
    }.toString()).toUpperCase();
    dio
        .get("/postService/allPost?sign=$sign",
  ...
  void searchPost() {
    ...
    if (searchTextController.value.text.isNotEmpty) {
      dio.get("/postService/searchPostByKeywordInTitleDescCategory?sign=$sign",
          queryParameters: {
            "currentPage": 0,
            "keyword": searchTextController.value.text,
            "pageSize": 100
  ...
  void postFormSubmit(Post post) {
    ...
    var sign=generateMd5(post.toJson().toString()).toUpperCase();
    dio.post("/postFormSubmit?sign=$sign", data: post.toJson()).then((value) {
  ...
  Future<void> searchPostNearMe() async {
    ...
    dio.get("/postService/postNearMe?sign=$sign", queryParameters: {
      "currentPage": 0,
      "lat": locationData?.latitude,
      "lon": locationData?.longitude,
      "distance": geoController.value.text,
...
Dio getDio() {
  var options = BaseOptions(
    baseUrl: 'http://192.168.86.128:8080/',
      // baseUrl:
